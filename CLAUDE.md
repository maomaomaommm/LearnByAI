...
### Local revision — fix + rewrite (`src/lib/maol/targetedRepair.ts`, `src/lib/repairAnchor.ts`, `src/lib/markdownSections.ts`, `src/lib/revisionApply.ts`)
Reading-time local edits run through the **Revise panel** (decoupled from the Tutor panel) and the **REVISER** agent. `POST /api/revisions` resolves the selection to a verbatim anchor (`resolveRevisionScopeAnchor`; scopes selection/paragraph/section), runs `fix` (minimal repair) or `rewrite` (directed rewrite/expansion that respects the chapter contract + course styles), and persists a `proposed` `Revision`; the `revise` quota is charged on the **proposal** (the LLM call), not on apply. `POST /api/revisions/apply` (by `revisionId`) replaces the anchor **exactly once**, re-runs deterministic TQH, and flips the revision to `applied`; `POST /api/revisions/[id]/revert` undoes it — a local text swap for selection/paragraph/section scopes, or a whole-chapter snapshot restore for regen. Whole-chapter regeneration snapshots the prior chapter (`snapshotChapterBeforeRegen`) at the server generate seam so it can be reverted. Revisions are user-confirmed; the model never silently overwrites the textbook.
