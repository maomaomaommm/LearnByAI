...
<p align="center">
  <img src="docs/assets/learnbyai-overview.png" alt="LearnByAI overview: from learning needs to Course Bible, multi-agent textbook generation, reader, tutor, reviser and continuous learning loop" width="960">
</p>

**Overview.** LearnByAI 将“用户学习需求 → Course Bible → 多 Agent 教材生成 → Reader/Tutor/Reviser → 持续学习迭代”组织成一条可追踪的个性化学习链路。
...
| 模块 | 它解决什么 | 当前能力 |
| --- | --- | --- |
| 📘 Course Bible | 避免学习内容零散 | 根据主题、目标、基础和风格生成课程蓝图与章节路线 |
| 🌐 Web Research | 避免快变领域内容过旧 | 可选检索近期论文与方法，再让 `ARCHITECT` 纳入大纲 |
| 🧩 Multi-Agent | 避免一个 Prompt 从头写到尾 | `ARCHITECT`、`AUTHOR`、`POLISHER`、`REVIEWER` 分工协作 |
| 🛡️ TQH Harness | 避免公式、结构和连续性崩坏 | 检查 Markdown、LaTeX、章节结构和教学连贯性 |
| 💬 Tutor | 避免教材读不懂只能重新问泛泛问题 | 选中文字或段落，基于当前课程上下文追问 |
| 🛠️ Reviser | 避免发现问题后只能整章重生 | 选区/段落/小节级改写，确认后精确替换，可撤销 |
| 🔁 Learning Loop | 避免本地玩具化 | Supabase、后台 Worker、SSE、配额、导出、管理后台 |
