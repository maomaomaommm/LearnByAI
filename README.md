...
**把一次性 AI 回答，变成可规划、可追问、可修复的个性化教材。**
...
![Beta](https://img.shields.io/badge/Status-Beta-8B5CF6?style=for-the-badge)

`Course Bible` · `Multi-Agent` · `TQH Quality Harness` · `Tutor` · `Reviser` · `Web Research`
...
```mermaid
flowchart TD
    Q["课程主题 + 学习目标"] --> K["生成英文检索关键词"]
    K --> E["Exa Search"]
    E -->|失败或未配置| S["StepFun Search"]
    S -->|失败或未配置| M["Kimi $web_search"]
    E --> R["研究摘要"]
    S --> R
    M --> R
    R --> A["ARCHITECT 规划大纲"]
    A --> T["requiredTopics 写入近期方法"]
```
...
sequenceDiagram
    ...
    participant W as AUTHOR
    participant P as POLISHER
    participant R as REVIEWER
    participant T as TQH

    U->>A: 主题、目标、背景、风格
    A->>A: 生成 Course Bible 与章节契约
    A->>W: 传递章节 requiredTopics
    W->>P: 章节草稿
    P->>T: 格式与结构检查
    T->>R: 质量报告
    R-->>U: ready / quality_failed / failed
```
...
flowchart TD
    ...
    B --> D["段落"]
    B --> E["小节"]
    C --> F["REVISER 生成 JSON 建议"]
    D --> F
    E --> F
    F --> G["展示 BEFORE / AFTER"]
    G --> H{"用户确认?"}
    H -->|否| I["保留原文"]
    H -->|是| J["后端精确匹配 beforeText"]
    J --> K["替换 afterText"]
    K --> L["重新质检并保存历史"]
```
