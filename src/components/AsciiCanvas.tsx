...
export default function AsciiCanvas({ forceDark = false }: { forceDark?: boolean }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      const isLight = !forceDarkRef.current && themeRef.current === 'light';
      ctx.fillStyle = isLight ? '#FAFAFA' : '#0A0A0A';
