...
function formatCourseMaterialsForChapter(course: Course) {
  const requirements = limitMaterial(course.courseRequirements, 5000);
  const references = limitMaterial(course.referenceMaterial, 7000);
  const style = limitMaterial(course.styleSample, 2500);
