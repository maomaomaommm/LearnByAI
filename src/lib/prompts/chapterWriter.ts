...
- “例”和“定义”必须使用普通教材段落，不得使用 Markdown 引用块。格式固定为「加粗的例 N.M：标题」或「加粗的定义 N.M：标题」，随后空一行写普通正文、表格和公式；不要使用 >、底色、左侧竖线或斜体来包裹它们。
...
:::
  - 禁止输出普通 Markdown 图片链接（例如 ![图示](images/x.png)、外部 URL 或转义后的 !\[...\]）；平台只能处理上述占位块。
- 有序步骤必须写成一个连续的 Markdown 列表，编号依次为 1.、2.、3.；每一项的解释、公式或段落必须缩进到该项下面。禁止把每一项都重新写成 1.，也不要在不同步骤之间另起一个独立列表。
